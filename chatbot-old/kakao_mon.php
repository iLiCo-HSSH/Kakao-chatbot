...
if ($t_day == 1){
  ...
  echo '{"version": "2.0",
    "template":
    {"outputs": [
      {"carousel": {"type": "basicCard","items": [{"title": "오늘 조식","description": "'. meal(1,0) .'"},{"title": "오늘 중식","description": "'. meal(2,0) .'"},{"title": "오늘 석식","description": "'. meal(3,0) .'"}]}},
      {"simpleText": {"text":"월요일 특) 나이스에서 급식을 잘 업데이트해주지 않음"}},
      {"textCard": {
          "title": "그래도 화요일부터는 정상!",
          "description": "월요일에는 조금 번거롭더라도 \n한성과고 홈페이지를 이용해 주세요!\n상단의 학교소식->학교급식->해당 날짜의 급식 클릭",
          "buttons": [
            {
              "action": "webLink",
              "label": "한성과고 홈페이지",
              "webLinkUrl": "https://hansungsh.sen.hs.kr/"
            }
          ]
      }
      }
  ...
  function mealTomorrow() {
    echo '{"version": "2.0",
      "template":
      {"outputs": [
        {"carousel": {"type": "basicCard","items": [{"title": "내일 조식","description": "'. meal(1,1) .'"},{"title": "내일 중식","description": "'. meal(2,1) .'"},{"title": "내일 석식","description": "'. meal(3,1) .'"}]}},
        {"simpleText": {"text":"월요일 특) 나이스에서 급식을 잘 업데이트해주지 않음"}},
        {"textCard": {
            "title": "그래도 화요일부터는 정상!",
            "description": "월요일에는 조금 번거롭더라도 \n한성과고 홈페이지를 이용해 주세요!\n상단의 학교소식->학교급식->해당 날짜의 급식 클릭",
